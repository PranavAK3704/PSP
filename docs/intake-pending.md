...
| **Human pass over ~800 `kapture_audits` rows** | Turns the 86.3% coverage figure from *silver* to *gold*. Until then it measures "the old classifier assigned a bucket", not "the taxonomy covers reality" | open | 2026-09-10 |
| **150–300 real off-topic messages** (the "play arena" class) | ⚠️ The evidence gate is **built but unmeasured on real negatives** — the 155-message corpus has only ~4 | open | 2026-09-10 |
...
| Dashboard: disposition, NOVEL queue, human confirm → exemplar | Closes the creep loop in the UI | next | 2026-09-10 |
